...
| The rate window lives in the backend when there is one | Counted per host, a deployment of two hosts allows twice the declared runs | A count of the local record store's runs is one query, but the claim is taken before a run row exists — the single-host fallback needs a table of its own for the same reason, `rate_starts`, written under the flock rather than derived from `runs` — and even that delivers FR-114 at half its declared strength on the deployments this feature exists for |
