...
| Gitea | yes | `X-Gitea-Signature` (also sent as `X-Gogs-Signature` and `X-Hub-Signature-256`) | hex, no prefix on the Gitea/Gogs headers; `sha256=<hex>` on the GitHub-compatible one | exact body | secret per webhook | `services/webhook/deliver.go`, function `addDefaultHeaders`, lines 97–147, commit `4d434455322a4780064ce67524ceb3711d865320` of `go-gitea/gitea`: `sig256 := hmac.New(sha256.New, secret)`; `io.MultiWriter(sig1, sig256).Write(payloadContent)`; `req.Header.Add("X-Gitea-Signature", signatureSHA256)` |
